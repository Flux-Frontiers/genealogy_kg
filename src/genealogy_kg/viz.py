...
#: printing. See :data:`SEX_SHAPE`: colour is never the only cue here.
...
def sex_kind(node: Mapping[str, Any]) -> str:
    ...
    only -- the store holds plain ``person`` -- which is exactly what
    ``GraphTheme.resolve_kind`` exists for.
...
def _generation_key(offset: int) -> str:
    ...
    if color_by == "sex":
        ...
        def resolve(node: Mapping[str, Any]) -> str:
            ...
            if offset is None:
                ...
                return sex_key
            return f"{_generation_key(offset)}_{sex_key}"
